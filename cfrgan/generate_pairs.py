...
class Estimator3D(object):
    def __init__(self, is_cuda=False, batch_size=1, render_size=224, test=True, model_path=None, back_white=False, cuda_id=0, det_net=None):
        self.is_cuda = False  # Force CPU mode
    ...
    def load_3dmm_models(self, model_path, test=True):
        # read face model
        self.face_model = BFM('mmRegressor/BFM/BFM_model_80.mat', -1)  # -1 for CPU
        ...
        if model_path is None:
            regressor.load_state_dict(torch.load("mmRegressor/network/th_model_params.pth", map_location='cpu'))
        else:
            regressor.load_state_dict(torch.load(model_path, map_location='cpu'))
        ...
        self.regressor = regressor.to('cpu')  # Ensure model is on CPU
    ...
    def reconstruct(self, coef, test=False):
        ...
        mesh = Meshes(verts=face_shape.to('cpu'), faces=self.tri[:face_shape.shape[0],...].to('cpu'), textures=Textures(verts_rgb=verts_rgb.to('cpu')))

        rendered = self.phong_renderer(meshes_world=mesh, R=self.R.to('cpu'), T=self.T.to('cpu'))
        rendered = torch.clamp(rendered, 0.0, 1.0)

        landmarks_2d = torch.zeros_like(face_projection, device='cpu')
    ...
    def estimate_five_landmarks(self, img):
        ...
        scale = torch.Tensor([img.shape[1], img.shape[0], img.shape[1], img.shape[0]], device='cpu')
        img -= (104, 117, 123)
        ...
        scale1 = torch.Tensor([img.shape[3], img.shape[2], img.shape[3], img.shape[2],
                            ...
                            img.shape[3], img.shape[2]], device='cpu')
    ...
    def get_occlusion_mask(self, model, scd, rot, gui, ori_img, obj=False):
        mean = torch.FloatTensor([0.485, 0.456, 0.406], device='cpu').unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
        std = torch.FloatTensor([0.229, 0.224, 0.225], device='cpu').unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
        ...
        coarse_occ_mask = torch.ones_like(guidance_noise, dtype=rot.dtype, device='cpu')
        ...
        eye_idx = (parsed==4).type(torch.BoolTensor) | (parsed==5).type(torch.BoolTensor)
        ...
        eye_idx = dilation(eye_idx.type(torch.FloatTensor), filter_size=5).type(torch.BoolTensor)
        coarse_occ_mask[eye_idx] = 0.0
        coarse_occ_mask[guidance_noise] = 0.0

        # Remove edge
        background = torch.zeros_like(guidance_noise, dtype=gui.dtype)
        ...
        edge = torch.zeros_like(background)
        edge[torch.logical_and(background>0.0, background<0.8)] = 1.0

        # Generate eye_mask
        eye_mask = torch.zeros_like(eye_idx, dtype=rot.dtype)
        ...
        if not obj:
            ...
            brow_lip_mask = torch.ones_like(eye_idx, dtype=rot.dtype)
    ...
    def swap_and_rotate_and_render(self, image, parsing_net, scd):
        ...
        angles = torch.rand(rot.shape[0], 3) # -pi/2 ~ pi/2
        ...
        rotated_shape = torch.matmul(front_face, rotation_m)
        ...
        # projection matrix
        p_matrix = _need_const.p_matrix.expand(rotated_shape.shape[0], 3, 3)
    ...
    def generate_testing_pairs(self, image, pose=[5.0, 0.0, 0.0], front=False, landmark=False):
        ...
        if landmark:
            ...
            landmarks_2d = torch.zeros_like(face_projection)
            landmarks_2d[...,0] = torch.clamp(face_projection[...,0].clone(), 0, self.render_size-1)
            landmarks_2d[...,1] = torch.clamp(face_projection[...,1].clone(), 0, self.render_size-1)
            landmarks_2d[...,1] = self.render_size - landmarks_2d[...,1].clone() - 1
            landmarks_2d = landmarks_2d[:,self.face_model.keypoints,:]
